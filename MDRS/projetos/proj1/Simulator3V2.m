function [PLd, PLv , APDd , APDv , MPDd, MPDv , APQDd, APQDv, TT] = Simulator3V2(lambda,C,f,P,n)
% INPUT PARAMETERS:
...
%  APQDd  - Average Packet Queuing Delay of data packets (milliseconds)
%  APQDv  - Average Packet Queuing Delay of VoIP packets (milliseconds)
...
% Queuing Delay Variables
TotalQueuingDelayD = 0;
TotalQueuingDelayV = 0;
...
while (TRANSMITTEDPACKETSD+TRANSMITTEDPACKETSV) < P               % Stopping criterium
    ...
    if Event == ARRIVAL         % If first event is an ARRIVAL
        ...
        if QUEUEOCCUPATION > 0
            ...
            if QUEUE(1,3) == DATA
                TotalQueuingDelayD = TotalQueuingDelayD + (Clock - QUEUE(1,2));
            else
                TotalQueuingDelayV = TotalQueuingDelayV + (Clock - QUEUE(1,2));
            end
...
APQDd = 1000*TotalQueuingDelayD / TRANSMITTEDPACKETSD;       % in milliseconds
APQDv = 1000*TotalQueuingDelayV / TRANSMITTEDPACKETSD;       % in milliseconds
